...
class HashTable(object):    # A hash table using open addressing
   ...
   def search(self, key):                # Get the value associated with a key in the hash table, if any
      ...
      return (None if (i is None) or     # return None if index is not found,
              self.__table[i] is None or # or item at i is empty,
              self.__table[i][0] != key  # or it has another key
              else self.__table[i][1])   # else return item value
